...
for (r in 1:R){
  ...
  for (j in 1:runlength){
    ...
    #################### Population 1
      ...
      K$Estoch[j,1] <- 1 #Probably better to calculate E(t=1) than to define it a priori
      ...
      Kb$Estoch[j,1] <- 1 #Probably better to calculate E(t=1) than to define it a priori
      ...
      Kd$Estoch[j,1] <- 1 #Probably better to calculate E(t=1) than to define it a priori
    ...
    ################################################################################################################### Iterate over years
      ...
      ###################################################################################################### Model Concessions of Population 1
       set.seed(666 + i + r*500) 
        ...
        # Cody's phase-in of taxation algorithm follows:
        # minC <- minTax
        # maxC <- maxTax
        # unconstrainedC <- (Kd$Nstoch[j,(i-1)] )/(K$Nstoch[j,(i-1)]) # seq(0,6,by=0.1) 
        # tax[i] <- ifelse(Kd$Nstoch[j,(i-1)]==0,0,(  2*(minC + (maxC-minC)*(logistic(L1+L2*unconstrainedC) - 0.5))  ) )
        
        # Start at maxTax per capita tax rate as soon as despots arise
        tax[i] <- ifelse(Kd$Nstoch[j,(i-1)]==0,0,maxTax)
       ...
       ###################################################################################################### Model Peaseants of Population 1
        ...
        K$Estoch[j,i] <- ((K$Ystoch[j,i]*Am*F.scrap)-sum(tax[i]*(K$nstoch[,(i-1),j])) )/(J*sum(K$nstoch[,(i-1),j]*rho))
        Tax[i]<-sum(tax[i]*(K$nstoch[,(i-1),j])) 
        ...
        # Draw tax from Peasants 1
         Kd$Estoch[j,i] <- (sum(tax[i]*(K$nstoch[,(i-1),j])) )/(Jd*sum(Kd$nstoch[,(i-1),j]*rhod))
      ...
      ################################################################################# Model IDD Dynamics        
         ...
         #### Allow Migration
         for(n in 1:N.AC){
         P1[n] <- P1[n]  -  (m*P1[n]*((Pi2[i]^v)/((Pi1[i])^v+((Pi2[i])^v))))  +  (m*P2[n]*(((Pi1[i])^v)/((Pi1[i])^v+((Pi2[i])^v)))) 

         P2[n] <- P2[n]  +  (m*P1[n]*((Pi2[i]^v)/((Pi1[i])^v+((Pi2[i])^v))))  -  (m*P2[n]*(((Pi1[i])^v)/((Pi1[i])^v+((Pi2[i])^v))))

         M1[n]  <-           (m*P1[n]*((Pi2[i]^v)/((Pi1[i])^v+((Pi2[i])^v))))  -  (m*P2[n]*(((Pi1[i])^v)/((Pi1[i])^v+((Pi2[i])^v))))
         M2[n]  <-           (m*P2[n]*(((Pi1[i])^v)/((Pi1[i])^v+((Pi2[i])^v)))) - (m*P1[n]*((Pi2[i]^v)/((Pi1[i])^v+((Pi2[i])^v))))
...
mean(Kd$Estoch[1,1000:2500])
mean(K$Estoch[1,1000:2500])
mean(Kb$Estoch[1,1000:2500])
