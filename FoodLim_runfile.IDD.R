...
K <- list(Ystoch=matrix(NA,ncol=span,nrow=runlength),NstochNM=matrix(NA,ncol=span,nrow=runlength),nstochNM=array(NA,dim=c(N.AC,span,runlength)), Nstoch=matrix(NA,ncol=span,nrow=runlength),nstoch=array(NA,dim=c(N.AC,span,runlength)),Estoch=matrix(NA,ncol=span,nrow=runlength))

Kb <- list(Ystoch=matrix(NA,ncol=span,nrow=runlength),NstochNM=matrix(NA,ncol=span,nrow=runlength),nstochNM=array(NA,dim=c(N.AC,span,runlength)),Nstoch=matrix(NA,ncol=span,nrow=runlength),nstoch=array(NA,dim=c(N.AC,span,runlength)),Estoch=matrix(NA,ncol=span,nrow=runlength))
...
for (r in 1:1){
  ...
  for (j in 1:runlength){
    ...
    ################################################################################################################### Iterate over years
      ...
      for(i in 2:span){
      ###################################################################################################### Model Pesants of Population 1
       set.seed(666 + i + r*500) 
        # Draw a yield from gamma distrib w mean=Y, CV=Y.CV and autocorrelation=Yac
        Ybar <- Y*(1-Yac)
        ...
        if( is.na(K$Estoch[j,(i-1)])){
          ...
          stop(paste(c("The 1st population in run ", j, " has crashed to extinction.")))
        }
        ...
        if( is.na(Kb$Estoch[j,(i-1)])){
          ...
          stop(paste(c("The 2nd population in run ", j, " has crashed to extinction.")))
        }
      ...
      ################################################################################# Model IDD Dynamics        
         ...
         M[n]  <-           (m*P1[n]*((Pi2[i]^v)/((c*Pi1[i])^v+((Pi2[i])^v))))  -  (m*P2[n]*(((c*Pi1[i])^v)/((c*Pi1[i])^v+((Pi2[i])^v))))
